...
null).executeQuery({success:function(d){for(var e={},f=0;f<d.rows.length;f++){var g=d.rows[f];e[g.tbl_name]=g}switch(c.providerConfiguration.dbCreation){case $data.storageProviders.DbCreationType.Merge:Guard.raise(new Exception("Not supported db creation type"));break;case $data.storageProviders.DbCreationType.DropTableIfChanged:d=[];for(f=0;f<c.SqlCommands.length;f++)if(""!=c.SqlCommands[f]){var h=/^CREATE TABLE IF NOT EXISTS ([^ ]*) (\(.*\))/g.exec(c.SqlCommands[f]);if(h&&(g=h[1],h=h[2],e[g.slice(1,
g.length-1)])){var i=/^CREATE TABLE ([^ ]*) (\(.*\))/g.exec(e[g.slice(1,g.length-1)].sql)[2];h.toLowerCase()!=i.toLowerCase()&&d.push("DROP TABLE IF EXISTS ["+e[g.slice(1,g.length-1)].tbl_name+"];")}}c.SqlCommands=c.SqlCommands.concat(d);break;case $data.storageProviders.DbCreationType.DropAllExistingTables:for(h in e)h&&!h.match("^__")&&!h.match("^sqlite_")&&c.SqlCommands.push("DROP TABLE IF EXISTS ["+e[h].tbl_name+"];")}c._runSqlCommands(b,{success:a.success,error:a.error})},error:a.error})},executeQuery:function(a,
...
save_DeleteEntity:function(a){for(var b="DELETE FROM ["+a.entitySet.name+"] WHERE(",c=!1,d=!1,e=[];!c;)a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(f){c&&!b.match(" AND $")&&(b+=" AND ");if(f.key||d)b+="(["+f.name+"] == ?)",e.push(this.fieldConverter.toDb[Container.resolveName(f.dataType)](a.data[f.name])),c=!0},this),c||(d=!0);b.match(" AND $")&&(b=b.slice(0,b.length-5));b+=");";return{query:b,param:e}},save_UpdateEntity:function(a){var b=" SET ",c="WHERE(",
d=!1,e=[],f=[];a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(g){void 0!==a.physicalData[g.name]&&(d&&!c.match(" AND $")&&(c+=" AND "),5<b.length&&!b.match(",$")&&(b+=","),g.key?(c+="(["+g.name+"] == ?)",e.push(this.fieldConverter.toDb[Container.resolveName(g.dataType)](a.physicalData[g.name])),d=!0):(b+="["+g.name+"] = ?",f.push(this.fieldConverter.toDb[Container.resolveName(g.dataType)](a.physicalData[g.name]))))},this);d||Guard.raise(new Exception("Not supported UPDATE function without primary key!"));
c.match(" AND $")&&(c=c.slice(0,c.length-5));b.match(",$")&&(b=b.slice(0,b.length-1));return{query:"UPDATE ["+a.entitySet.tableName+"]"+b+" "+c+");",param:f.concat(e)}},save_NewEntity:function(a){var b="INSERT INTO ["+a.entitySet.tableName+"](",c="",d="",e=[];a.physicalData.constructor.memberDefinitions.getPublicMappedProperties().forEach(function(b){0<c.length&&","!=c[c.length-1]&&(c+=",",d+=",");var g=b.name;if(void 0!==a.physicalData[g]&&b.dataType&&(!b.dataType.isAssignableTo||b.dataType.isAssignableTo&&
!b.dataType.isAssignableTo($data.EntitySet)))d+="?",c+="["+g+"]",e.push(this.fieldConverter.toDb[Container.resolveName(b.dataType)](a.physicalData[g]))},this);1>e.length&&Guard.raise(new Exception("None of the fields contain values in the entity to be saved."));","==c[c.length-1]&&(c=c.slice(0,c.length-1));","==d[d.length-1]&&(d=d.slice(0,d.length-1));return{query:b+(c+") VALUES("+d+");"),param:e}},save_reloadSavedEntity:function(a,b){return{query:"SELECT * FROM "+b+" WHERE rowid=?",param:[a]}},createSqlFromStorageModel:function(a){(void 0===
a||null===a||void 0===a.PhysicalType)&&Guard.raise("StorageModel not contains physical entity definition");var b=0,c=0;a.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(a){a.key&&b++;a.computed&&c++},this);1===c&&1<b&&Guard.raise(new Exception("Do not use computed field with multiple primary key!"));1<c&&1<b&&Guard.raise(new Exception("Do not use multiple computed field!"));var d="CREATE TABLE IF NOT EXISTS ["+a.TableName+"] (",e=",PRIMARY KEY (";a.PhysicalType.memberDefinitions.getPublicMappedProperties().forEach(function(b,
